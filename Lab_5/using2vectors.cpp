...
{
	ch=0;
	itm=find(mpk.begin(), mpk.end(), s);
	if(itm!=mpk.end())return mpv[itm-mpk.begin()];
	valid++;ch++;
	return 0;
}
...
int main()
{
    ...
    if(itv != v.end())
    {
    	...
    	int val=eval(root);
    	itm=find(mpk.begin(), mpk.end(), v[0]);
    	if(itm!=mpk.end()){mpv[itm-mpk.begin()]=val;}
    	else{mpk.push_back(v[0]);mpv.push_back(val);}
    }
    else
    {
    int flag=0;
